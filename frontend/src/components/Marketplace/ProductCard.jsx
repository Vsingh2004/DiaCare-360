...
const ProductCard = ({ product }) => {
  ...
  const toggleFavorite = () => {
    ...
    if (isFavorite) {
      favorites = favorites.filter((id) => id !== product._id); // Remove from favorites
    } else {
      favorites.push(product._id); // Add to favorites
    }
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className={`absolute top-2 right-2 p-2 ${isFavorite ? "text-red-500" : "text-gray-400"}`}
      >
        <svg
        ...
        <div className="flex gap-2">
          ...
          <Button
            ...
            className={`flex items-center gap-1 ${
              addedToCart ? "bg-green-500" : "bg-[#046c4e]"
            } text-white hover:bg-[#035c41] px-3 py-1.5`}
      ...
      <div>
        <Button
          size="xl"
          className="w-full flex items-center gap-1 mt-6 bg-[#0fc994] text-white hover:bg-[#035c41] px-3 py-1.5"
        >
          Add to your Planner
        </Button>
      </div>
