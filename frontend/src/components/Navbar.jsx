...
const Navbar = () => {
  ...
  const { cartItems } = useCart(); // Importing the cart from CartContext

  const cartCount = cartItems?.length || 0; // Get the number of items in the cart

  useEffect(() => {
    // Simulate getting user from localStorage or context
    const token = localStorage.getItem("user");
    if (token) {
      setUser({ token }); // or decode it if needed
    }
  }, []);
    ...
    <div className="fixed top-0 left-0 w-full bg-white shadow-md px-8 py-5 z-50">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-5 items-center">
          {/* Show Favourites and Cart only if user is logged in */}
          {user && (
            <>
              {/* Favourites */}
              <a href="/favourites">
